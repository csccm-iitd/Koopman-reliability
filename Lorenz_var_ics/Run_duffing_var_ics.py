...
ff.train(model, data_1, data_2, num_epochs=300, shedule=True, criterion =nn.MSELoss(), title=None)
...
#%%computing reliability or probabilty of failure
data_tr1= np.array(test_tr1).reshape(10000,100,3)
data_tr2= np.array(test_tr2).reshape(10000,100,3)

#test
p_f = ff.failurefunction(data_tr2,[20, 20, 25],10000,30)
print(p_f)

# prediction
predict2=np.array(predict2)
p_f_p = ff.failurefunction(predict2,[20, 20, 25],10000,30)

#%% PDF of the fiorst passagea failure time
k1 = ff.densityfunction(predict2,[20,20,25],10000,100)
k2 = ff.densityfunction(data_tr2,[20,20,25],10000,100)

t_data = np.array(t_d[1:101])

kde = gaussian_kde(t_data[k1,0] )
dist_space = linspace( -2, 2, 100 )
