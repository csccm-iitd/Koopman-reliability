...
def train(model, dataloader1, dataloader2, num_epochs, shedule=True, criterion =nn.MSELoss(), title=None):
...
def failurefunction(y0,Lt,N,T_step):
    count=0;
    y1=y0[:,0:T_step,0];y2=y0[:,0:T_step,1];y3=y0[:,0:T_step,2];
    for i in range(0,N):
       ja1=np.where(abs(np.array(y1)[i,:])>Lt[0])[0]
       ja2=np.where(abs(np.array(y2)[i,:])>Lt[1])[0]
       ja3=np.where(abs(np.array(y3)[i,:])>Lt[2])[0]
       ja=np.concatenate([ja1,ja2,ja3]).tolist()
       if len(ja)!=0 :      
          count=count+1
    Pf=count/N
    return Pf
